...
define(function(require,exports,module){
	...
	/**
	 * Responsive ResponsiveTables helper init function.  Builds breakpoint limits
	 ...
	 */
	ResponsiveTable.prototype.init = function (breakpoints) {
	...
	/**
	 * Respond window size change.  This helps make ResponsiveTables responsive.
	 */
	    ...
	    if (this.columnsHiddenIndexes.length) {
	        ...
	        $('tr.detail-show', this.tableElement).each(function (index, element) {
	            ...
	            if (tr.next('.row-detail').length === 0) {
	                ResponsiveTable.prototype.showRowDetail(that, tr);
	            }
	        });
	    } else {
	...
	/**
	 * Show/hide ResponsiveTables columns.
	 */
	ResponsiveTable.prototype.showHideColumns = function () {
	    ...
	    if (this.tableElement.hasClass('has-columns-hidden')) {
	        $('tr.detail-show', this.tableElement).each(function (index, element) {
	            ResponsiveTable.prototype.showRowDetail(that, $(element));
	        });
	    }
	};
	 ...
	 */
	ResponsiveTable.prototype.createExpandIcon = function (tr) {
	    if (this.disabled) {
	        return;
	    }
	    ...
	    if (this.expandColumn !== undefined && this.expandColumn < tds.length) {
	        ...
	        if ($('span.responsiveExpander', td).length == 0) {
	            ...
	            td.on('click', 'span.responsiveExpander', {ResponsiveTableInstance: this}, this.showRowDetailEventHandler);
	        }
	    }
	};
	 ...
	 */
	ResponsiveTable.prototype.showRowDetailEventHandler = function (event) {
	    if (this.disabled) {
	        return;
	    }
	    ...
	    if (tr.hasClass('detail-show')) {
	        ResponsiveTable.prototype.hideRowDetail(event.data.ResponsiveTableInstance, tr);
	    } else {
	        ResponsiveTable.prototype.showRowDetail(event.data.ResponsiveTableInstance, tr);
	    }
	 ...
	 * Show row details
	 *
	 * @param {ResponsiveTable} ResponsiveTableInstance instance of ResponsiveTable
	 * @param {Object}                     tr                                 jQuery wrapped set
	 */
	ResponsiveTable.prototype.showRowDetail = function (ResponsiveTableInstance, tr) {
	    // Get column because we need their titles.
	    var tableContainer = ResponsiveTableInstance.tableElement;
	    var columns = tableContainer.fnSettings().aoColumns;

	    // Create the new tr.
	    var newTr = $(ResponsiveTableInstance.rowTemplate);

	    // Get the ul that we'll insert li's into.
	    var ul = $('ul', newTr);

	    // Loop through hidden columns and create an li for each of them.
	    _.each(ResponsiveTableInstance.columnsHiddenIndexes, function (index) {
	        var li = $(ResponsiveTableInstance.rowLiTemplate);
	    ...
	    var colspan = ResponsiveTableInstance.columnIndexes.length - ResponsiveTableInstance.columnsHiddenIndexes.length;
	 ...
	 * @param {Object}                     tr                                 jQuery wrapped set
	 */
	ResponsiveTable.prototype.hideRowDetail = function (ResponsiveTableInstance, tr) {
	 ...
	 */
	ResponsiveTable.prototype.disable = function (disable) {
